...
import { createChangeDestinationInstruction, createCreateInstruction, createInitInstruction, createUnlockInstruction } from './instructions';
import {
  connection,
  account,
  VESTING_PROGRAM_ID,
...
export async function changeDestination(
  ...
  if (newDestinationTokenAccount == undefined){
    assert(newDestinationTokenAccountOwner != undefined, 'At least one of newDestinationTokenAccount and newDestinationTokenAccountOwner must be provided!');
    newDestinationTokenAccount = await findAssociatedTokenAddress(newDestinationTokenAccountOwner!, contractInfo.mintAddress);
  }
...
const test = async (): Promise<void> => {
  ...
  const instructions = await create(
    VESTING_PROGRAM_ID,
    [Buffer.from('1111111111111481234512345123451211111111111114512345123451234512', 'hex')],
    account,
    account,
    tokenPubkey,
    destinationPubkey,
